...
void Load_Bios(string test)
{
	...
	char Ram[FileSize];                       //New char array the size of the rom file
	MemoryFile.read(Ram, FileSize);           //Read the file
	MemoryFile.close();                       //Close the file

	//Jump code to be written to 0xFFFFF, =JMP FAR 0xF000:0X0100
	char FFFF0[] = {0XEA, 0X00, 0X08, 0X00, 0X00, 'E', 'M', ' ', '0', '4', '/', '1', '0', '/', '2', '0'};
	Write_Memory_Array(0xFFFF0, FFFF0, sizeof(FFFF0)); //Jump Code
	Write_Memory_Array(0x00000, Ram, sizeof(Ram));     //The Ram file
}
